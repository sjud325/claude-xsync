...
확정 의미론: **역검증 실패 시 1단계 안정성 검사로 강등한다.** `d2 = normalize(resolved)`가 자체 왕복 고정점이면(`normalize(resolve_pull(d2)) == d2`) 적용(+알림), 아니면 스킵(진짜 손상 의심).
- 통과 조건이 곧 이후 왕복의 수렴(고정점) 증명이므로 손상 감지력은 유지된다.
- 검사 형태 확정 근거(Windows CI 실증): pull 출력은 슬래시 캐논 형태이므로, 더 엄격한 바이트 비교(`resolve_pull(d2) == resolved`)로는 **백슬래시 형태 인용**(맥 세션에 붙여넣는 Windows 로그의 통상 형태)이 Windows 기기에서 절대 흡수되지 못한다 — 주 사용 방향이 막히므로 고정점 형태로 완화.
- 결과: 인용 텍스트는 **첫 홉에 바이트 원문 그대로 도착**하고, 이후 홉에서 케이스·구분자가 수신 기기의 캐논 형태로 수렴하며 여느 경로처럼 안정 번역된다(예: 맥 세션 속 `C:\Users\Loki\x` 인용이 한 사이클 뒤 맥에서 `/Users/woong/x`로 보임). 기계 소비 경로(cwd·백업 키)는 전 구간 무손상.
