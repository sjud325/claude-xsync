...
- **상대 OS 홈 인용은 보존됨**: 각 기기는 자기 홈 형태만 변환한다(의도된 의미론). 인용된 상대 기기 경로가 "그 기기에서 유효한 경로"로 재해석되기를 기대할 수 없음.
...
## 11. v2 파킹랏
`~/.claude.json`의 `projects` dict 선별 싱크(신뢰/허용도구/프로젝트별 MCP), 동기화 폴더 백엔드, brew/Scoop, SessionEnd/Start 훅 자동화(수동으로 충분해질 때까지 보류), 스냅샷 모드 기본화 검토, MCP per-OS override 문법.
